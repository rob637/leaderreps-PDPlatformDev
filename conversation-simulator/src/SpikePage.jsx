...
import { Mic, Square, Activity, AlertTriangle, Loader2, Play, LogOut } from 'lucide-react';
import { SCENARIOS, DEFAULT_SCENARIO_ID } from './lib/scenarios.js';
import { createAudioPipeline } from './lib/audio.js';
import { connectLiveSession } from './lib/geminiLive.js';
import { mintSimulatorToken, signOutUser } from './auth.jsx';
...
const MAX_DURATION_MS = 6 * 60 * 1000; // 6 min auto-stop

export default function SpikePage({ user, mode }) {
  const [scenarioId, setScenarioId] = useState(DEFAULT_SCENARIO_ID);
  const [state, setState] = useState(STATE.IDLE);
  const [errorMsg, setErrorMsg] = useState(null);
  const [transcript, setTranscript] = useState([]); // [{role, text, final}]
  ...
  async function start() {
    ...
    // Resolve credentials: prefer ephemeral token in auth mode; fall back to
    // local API key only when no Firebase config is present.
    let ephemeralToken = null;
    let apiKeyForSession = null;
    if (mode === 'auth') {
      try {
        const minted = await mintSimulatorToken();
      ...
      apiKeyForSession = localApiKey;
    } else {
      setErrorMsg('No Firebase config and no VITE_GEMINI_API_KEY — cannot connect.');
      setState(STATE.ERROR);
      return;
    }
    ...
    try {
      ...
      sessionRef.current = await connectLiveSession({
        ...
        onInputTranscript: (text, final) => {
          ...
          if (final) {
            // End of user turn — start measuring TTFB for next AI response.
        ...
        onOutputTranscript: (text, final) => {
          ...
          if (final) pendingOutputRef.current = '';
        },
        onInterrupted: () => {
          // User barged in — flush queued AI audio so we don't talk over ourselves.
          audioRef.current?.flushPlayback();
        },
        onTurnComplete: () => { /* no-op for spike */ },
        onError: (err) => {
      ...
      <header className="bg-corporate-navy text-white">
        <div className="max-w-5xl mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold mt-1">Phase 0 — Latency Spike</h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            {user && (
              <div className="hidden sm:flex items-center gap-2 text-xs text-white/70">
                ...
                <button onClick={() => signOutUser()} className="hover:text-white" title="Sign out">
...
function ScenarioCard({ scenario, disabled, allScenarios, onChange }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
      ...
      <p className="text-sm text-slate-700 leading-relaxed">{scenario.framingText}</p>
      <div className="mt-4 pt-4 border-t border-slate-100">
        ...
        <div className="flex flex-wrap gap-2">
          {scenario.coachingFocus.map((f) => (
            <span key={f} className="text-xs px-2 py-1 bg-slate-100 text-slate-700 rounded-md">
              {f}
            </span>
          ))}
        </div>
...
function TranscriptPanel({ transcript, state }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 min-h-[320px]">
...
function MetricsPanel({ metrics, avgRoundTrip, state }) {
  ...
  // Live audio cost estimate (rough): Gemini 2.0 Flash Live ≈
  //   audio input ~$0.0001/sec, audio output ~$0.0004/sec.
  // Use bytes-as-time proxy: 16kHz mono PCM16 = 32,000 bytes/sec; 24kHz = 48,000 bytes/sec.
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-bold">Spike metrics</h3>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 text-[11px] text-slate-500 leading-relaxed">
        ...
        Either &gt; switch to OpenAI Realtime before Phase 1.
      </div>
    </div>
  );
}
...
function Metric({ label, value, warn }) {
  return (
    <div className="flex items-baseline justify-between gap-3">
      ...
      <dd className={`font-mono font-semibold ${warn ? 'text-red-600' : 'text-slate-900'}`}>
        {value}
      </dd>
    </div>
  );
...
function SpikeNotice() {
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900 leading-relaxed">
      ...
      Admin-only. Each session mints a short-lived (10 min) ephemeral Gemini
      Live token via Cloud Function. No long-lived API key in the browser.
    </div>
  );
}
