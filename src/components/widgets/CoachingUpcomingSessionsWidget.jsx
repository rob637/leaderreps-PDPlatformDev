...
import { Calendar, Clock, Users, MapPin, Video, ChevronRight } from 'lucide-react';
import { Card } from '../ui';
...
const CoachingUpcomingSessionsWidget = ({ scope = {} }) => {
  ...
  const displaySessions = upcomingSessions.length > 0 
    ? upcomingSessions 
    : sessions.filter(s => {
      ...
      <div className="space-y-3">
        {displaySessions.slice(0, 5).map(session => (
          <SessionCard 
            key={session.id}
            session={session}
            isRegistered={registeredIds.has?.(session.id) || registeredIds[session.id]}
