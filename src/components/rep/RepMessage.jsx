  ...
  if (isSystem) {
    return (
      <div className="flex justify-center my-4">
        <div className="text-xs text-rep-text-secondary bg-rep-navy-light px-4 py-2 rounded-full">
          {content}
        </div>
      </div>
    );
    ...
    <div className={`flex gap-3 mb-4 ${isRep ? 'justify-start' : 'justify-end'}`}>
      ...
      <div 
        className={`
          ...
          ${isRep 
            ? 'bg-rep-navy-light text-rep-text-primary rounded-2xl rounded-tl-md' 
      ...
      >
        ...
        {/* Timestamp (optional) */}
        {timestamp && !isTyping && (
          <p className={`text-xs mt-2 ${isRep ? 'text-rep-text-secondary' : 'text-white/70'}`}>
            {timestamp}
          </p>
        )}
      </div>
