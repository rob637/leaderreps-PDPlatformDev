...
const DataUploadComponent = ({ onDataParsed, expectedFields, isSaving }) => {
  ...
  const handleJsonPaste = () => {
    try {
        ...
        // Call the parent component's data staging function
        onDataParsed(raw);
        setStatus('success');
        setShowJsonInput(false);
    } catch (error) {
...
const ScenariosTableEditor = ({ data, isSaving, setGlobalData, idKey = 'id' }) => {
  ...
  const handleScenarioDataParsed = useCallback((parsedData) => {
    setGlobalData(prevGlobal => {
      ...
      // Filter out existing IDs before merging
      const newScenarios = parsedData.filter(s => !existingIds.has(s.id));
      alert(`Mass load: ${newScenarios.length} new scenarios staged.`);
...
const TargetRepTableEditor = ({ data, isSaving, setGlobalData, idKey = 'id' }) => {
  ...
  const handleTargetRepDataParsed = useCallback((parsedData) => {
    setGlobalData(prevGlobal => {
      ...
      return { ...prevGlobal, TARGET_REP_REP: [...(prevGlobal.TARGET_REP_CATALOG || []), ...newReps] };
    });
  }, [setGlobalData]);
...
const GlobalDataEditor = ({ globalMetadata, updateGlobalMetadata, db, navigate }) => {
  ...
  const renderTabContent = () => {
    // Renders the content based on the selected tab (currentTab)
    switch (currentTab) {
      case 'reading':
            ...
            <div className='flex flex-wrap gap-2'>
              {items.map(item => (
                <button
                  ...
                  // FIX: Correctly reference currentTab for active state
                ...
                >
                  ...
                  {item.count !== undefined && 
                    // FIX: Correctly reference currentTab for badge color
                    <span className={`ml-1 px-1.5 py-0.5 text-xs rounded-full ${item.key === currentTab ? 'bg-white text-[#002E47]' : 'bg-gray-200 text-gray-700'}`}>{item.count}</span>
                  }
                </button>
              ))}
            </div>
