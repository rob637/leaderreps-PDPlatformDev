...
async function handleAiSubmit(e, services, selectedBook, aiQuery, setIsSubmitting, setAiResponse) {
  ...
  try {
    ...
    const payload = {
        ...
        model: services.GEMINI_MODEL || 'gemini-2.5-flash',
                          ...
                          >
                            ...
                            <div className="flex-grow pt-2">
                                ...
                                <div className="mt-3 pt-3 border-t border-slate-100">
                                    <p className="text-[10px] font-semibold uppercase mb-1.5 text-corporate-teal-ink">Key Focus</p>
...
export default function BusinessReadingsScreen() {
  ...
  useEffect(() => {
    ...
    if (autoOpenId && !isLoadingCms && Object.keys(allBooks).length > 0 && !selectedBook) {
      ...
      for (const tier in allBooks) {
        const books = allBooks[tier];
        const match = books.find(b => String(b.id).toLowerCase() === targetId);
        if (match) {
