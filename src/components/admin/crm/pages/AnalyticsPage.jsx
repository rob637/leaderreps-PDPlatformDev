...
export default function AnalyticsPage() {
  ...
  const { prospects, subscribeToProspects, isCurrentUserAdmin, setCurrentUser, getProspectsByOwner } = useProspectsStore();
  ...
  }, [subscribeToProspects, initialize, cleanup, activeEmail, setCurrentUser]);

  const isAdmin = isCurrentUserAdmin();
  
  // Filter prospects by active persona
  const userProspects = activeEmail ? getProspectsByOwner(activeEmail) : prospects;
  ...
  // Pipeline stats - filtered by active persona
  const pipelineStages = PIPELINE_STAGES.filter(s => !['won', 'lost'].includes(s.id)).map(stage => ({
    ...stage,
    count: userProspects.filter(p => p.stage === stage.id).length
  }));
  
  const activeProspectsCount = userProspects.filter(p => !['won', 'lost'].includes(p.stage)).length;
  const wonProspectsCount = userProspects.filter(p => p.stage === 'won').length;
  const pipelineValue = userProspects
    .filter(p => !['won', 'lost'].includes(p.stage))
    .reduce((sum, p) => sum + (p.value || 0), 0);
  const wonValue = userProspects
    .filter(p => p.stage === 'won')
    .reduce((sum, p) => sum + (p.value || 0), 0);
  
  // Team leaderboard (admin only) - uses all prospects
  ...
  // Stale prospects (no activity in 7+ days) - filtered by active persona
  const staleProspectsCount = userProspects.filter(p => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subValue={`${staleProspectsCount} need attention`}
          color="bg-amber-500"
        />
        <StatCard
          icon={DollarSign}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
          ...
          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 grid grid-cols-2 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-green-600">{wonProspectsCount}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400">Won</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-slate-400">{userProspects.filter(p => p.stage === 'lost').length}</div>
        ...
        {staleProspectsCount > 0 && (
          <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800 p-6">
            ...
            <p className="text-amber-700 dark:text-amber-400 mb-3">
              {staleProspectsCount} prospects haven't been contacted in 7+ days
            </p>
            <button
