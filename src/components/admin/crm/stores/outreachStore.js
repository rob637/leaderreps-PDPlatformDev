...
export const useOutreachStore = create((set, get) => ({
  ...
  // Initialize real-time subscriptions - filter by user email
  initialize: (userEmail) => {
    const { unsubscribers } = get();
    const canonicalEmail = userEmail ? getCanonicalEmail(userEmail) : null;
    
    // Store current user email for filtering
    set({ currentUserEmail: canonicalEmail });
    ...
    // Subscribe to templates - filter by owner email if provided
    set({ templatesLoading: true });
    const templatesQuery = canonicalEmail 
      ? query(
    ...
    // Subscribe to sequences - filter by owner email if provided
    set({ sequencesLoading: true });
    const sequencesQuery = canonicalEmail
      ? query(
    ...
    // Subscribe to recent activities - filter by owner email if provided
    set({ activitiesLoading: true });
    const activitiesQuery = canonicalEmail
      ? query(
  ...
  // Template actions
  createTemplate: async (templateData, userId, userName, userEmail) => {
    try {
      const canonicalEmail = userEmail ? getCanonicalEmail(userEmail) : null;
  ...
  // Sequence actions
  createSequence: async (sequenceData, userId, userName, userEmail) => {
    try {
      const canonicalEmail = userEmail ? getCanonicalEmail(userEmail) : null;
  ...
  // Activity actions
  logActivity: async (activityData, userId, userName, userEmail) => {
    try {
      const canonicalEmail = userEmail ? getCanonicalEmail(userEmail) : null;
