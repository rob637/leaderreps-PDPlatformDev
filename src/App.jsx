...
import {
  getFirestore,
  setLogLevel,
  // NEW: expose these so we can use them in console via window.__fb
  doc,
  getDoc,
  setDoc,
  updateDoc,
...
const DataProvider = ({ children, firebaseServices, userId, isAuthReady, navigate, user }) => {
  ...
  // Display initial loading spinner if global metadata is still loading
  if (isLoading) {
    return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className={`p-8 bg-white rounded-xl shadow-2xl text-center w-full max-w-sm border-t-4 border-[${TEAL}]`}>
        ...
        <div className='mt-6 border-t pt-4 border-gray-200 space-y-2'>
          {mode !== 'signup' && (
            <button onClick={() => { setMode('signup'); setStatusMessage(''); }} className={`text-sm text-[${TEAL}] hover:text-[${NAVY}] font-semibold block w-full`}>Need an account? Sign up</button>
          )}
          {mode !== 'login' && (
            <button onClick={() => { setMode('login'); setStatusMessage(''); }} className={`text-sm text-gray-500 hover:text-[${NAVY}] block w-full`}>Back to Sign In</button>
          )}
          {mode === 'login' && (
            <button onClick={() => { setMode('reset'); setStatusMessage(''); }} className={`text-sm text-gray-500 hover:text-[${ORANGE}] block w-full`}>Forgot Password?</button>
          )}
        </div>
      </div>
    </div>
...
const ScreenRouter = ({ currentScreen, navParams, navigate }) => {
  ...
  if (currentScreen === 'app-settings') {
    return <AppSettingsScreen key={uniqueKey} navigate={navigate} />;
  }
  if (currentScreen === 'data-maintenance') {
    return <Component key={uniqueKey} navigate={navigate} />;
  }
...
const App = ({ initialState }) => {
  ...
  // CRITICAL FIX 5: useCallback so navigate is stable
  ...
  // NEW: simple waiter for console usage
  useEffect(() => {
    if (typeof window === 'undefined') return;
    window.__fbReady = () => new Promise((resolve) => {
      const check = () => {
        const ok = !!(firebaseServices?.db && firebaseServices?.auth);
        if (ok) resolve(window.__fb);
  ...
  // NEW: attach console helpers once db/auth are ready
  useEffect(() => {
    if (typeof window === 'undefined') return;
    ...
    const fb = {
      ...
      auth,
      db,
      // Re-export Firebase helpers so the console has them
      getAuth,
      getFirestore,
      doc, getDoc, setDoc, updateDoc, collection,
      serverTimestamp, arrayUnion, arrayRemove, onSnapshot,
      uid: () => auth.currentUser?.uid || null,
      ensureUserDocs: async (uid) => ensureUserDocs(db, uid || auth.currentUser?.uid),
      // One-shot quick check for the three user docs
      testReads: async () => {
        const uid = auth.currentUser?.uid;
        if (!uid) throw new Error('No currentUser; sign in first.');
        const r = await getDoc(doc(db, 'leadership_plan', uid, 'profile', 'roadmap'));
        const a = await getDoc(doc(db, 'user_commitments', uid, 'profile', 'active'));
        const d = await getDoc(doc(db, 'user_planning', uid, 'profile', 'drafts'));
        const out = {
          roadmap: { exists: r.exists(), data: r.data() },
          active: { exists: a.exists(), data: a.data() },
          drafts: { exists: d.exists(), data: d.data() },
        };
  ...
  if (!DEBUG_MODE) {
    ...
    if (!user && isAuthReady) {
      return (
        <React.Fragment>
          <AuthPanel auth={firebaseServices.auth} onSuccess={() => { setAuthRequired(false); setTimeout(() => navigate('dashboard'), 0); }}/>
        </React.Fragment>
      );
    }
  }
