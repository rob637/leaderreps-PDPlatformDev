...
import { getFirestore, setLogLevel, doc, getDoc, setDoc, onSnapshot, writeBatch, updateDoc, arrayUnion, arrayRemove, increment, serverTimestamp, addDoc, collection } from 'firebase/firestore';
// ---- DEBUG/CONSOLE HELPERS (injected) -----------------------------------------
// Expose Firebase helpers and a tiny toolbox on window for easy console testing.
try {
  // Safe attach without clobbering existing
  ...
  // Wait until auth+db are ready (user may already be signed in)
  window.fbReady = async function fbReady() {
    const auth = getAuth();
    // If a user is already there, we're ready
    if (auth.currentUser) return true;
    // Otherwise, wait once for auth state resolution
  ...
  // Seed/repair the three required docs if missing
  window.fb = window.fb || {};
  window.fb.ensureUserDocs = async function ensureUserDocs() {
    ...
    const paths = [
      ['leadership_plan',  uid, 'profile', 'roadmap',  { plan_goals: [], last_updated: new Date().toISOString() }],
      ['user_commitments', uid, 'profile', 'active',   { active_commitments: [], reflection_journal: '' }],
    ...
    for (const [coll, u, prof, docId, seed] of paths) {
      ...
      if (!snap.exists()) {
        ...
        // Small noop write to bump listeners
        await updateDoc(ref, { __touched: serverTimestamp() }).catch(()=>{});
      }
    }
  };

  // Read & log all three docs
  window.fb.checkUserDocs = async function checkUserDocs() {
    ...
    const out = { roadmap: r.data() || null, active: a.data() || null, drafts: d.data() || null };
    console.log('[TEST READS]', out);
    return out;
  };

  // Convenience: live listeners to verify UI reactivity from the console
  window.fb.liveTest = async function liveTest() {
    ...
    if (!uid) throw new Error('No signed-in user');
    const unsubs = [
      onSnapshot(doc(db,'leadership_plan',uid,'profile','roadmap'), s => console.log('[LIVE] roadmap', s.exists(), s.data())),
      onSnapshot(doc(db,'user_commitments',uid,'profile','active'), s => console.log('[LIVE] active', s.exists(), s.data())),
      onSnapshot(doc(db,'user_planning',uid,'profile','drafts'), s => console.log('[LIVE] drafts', s.exists(), s.data())),
    ];
            ...
            <div className='space-y-4'>
                <form onSubmit={(e) => { e.preventDefault(); handleAction(); }}>
                    ...
                    <input 
                        ...
                        // CRITICAL FIX: Typo corrected here
